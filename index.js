...
// ── 啟動背景執行緒 ────────────────────────────────────
const worker = new Worker(join(__dirname, 'plc-worker.js'));
...
console.log('=== PLC Listener ===');
console.log('背景執行緒啟動中，等待 PLC 連入 (port 8100)...');
printHelp();
...
rl.on('line', (line) => {
  ...
  if (!plcConnected) {
    console.log('  [!] 尚未有 PLC 連線，請等待 PLC 連入');
    rl.prompt();
    return;
  }

  if (waitingResponse) {
    console.log('  [!] 等待上一筆回應中...');
    rl.prompt();
    return;
  }
